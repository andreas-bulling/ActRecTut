...
clear;
evalType = {'pd', 'pi'};
nFeatures = [1 5 10 20 25 50 150 200 250 300];
...
for nF = nFeatures
    for ec =  evalType
        for s =  [1 2]
            ...
            SETTINGS.FEATURE_SELECTION_OPTIONS = nF;
            ...
            [confusion, metrics, scoreEval fSelection] = run_evaluation(features, fType, fDescr, segments, segmentation, labelsSegmentation, featureLabels, SETTINGS);
               % store results
            filename = saveExperiment(confusion, metrics, scoreEval, mfilename, [num2str(nF) 'mRMR'], SETTINGS,'fSelection', fSelection,'featureLabels',featureLabels);   
        end
    end
end
