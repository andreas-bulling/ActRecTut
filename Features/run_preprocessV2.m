...
function [features fType fDescr segments segmentation labelsSegmentation, featureLabels] = run_preprocessV2(dataAll, labelsAll, segmentsAll, SETTINGS)
...
if SETTINGS.PLOT  
    ...
    dataAll = selectSensorData(dataAll, 'sensorsAvailable', SETTINGS.SENSORS_AVAILABLE, 'sensorsUsed', SETTINGS.SENSORS_USED, ...
     'fusion', SETTINGS.FUSION_TYPE, 'verbose', SETTINGS.VERBOSE_LEVEL);
  dim = size(dataAll, 2);
    for i = 1:6:dim
        ...
        for s=0:-1
            ...
            hold all;
            plot(Xsensor);
            title(sprintf('%s %s', SETTINGS.SENSORS_USED{s+1}, SETTINGS.SENSORS_USED{p})); 
        end  
        p=p+1;
    end
